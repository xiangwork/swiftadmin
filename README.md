...
本项目开源协议为Apache2，版权号为 `2021SR0761953`、因近期发现有直接抄袭本项目的、包括但不限于后台UI、我觉得即是开源，就应该为大多数人提供方便，但是我们不能直接拿别人的代码扣到自己的项目里去做开源，这是极其不尊重开源协议的行为。这是我们不能容忍的。SAPHP开源项目的开发，肯定也会借鉴别人框架的优点，站在巨人的肩膀上才能走的更远。但SAPHP一直秉承开源学习的是思路，代码自己去实现这一观点、既可以让开源项目发挥他的本质、又可以相互学习到东西！

PS：本项目的开源包含很多清晰的注释，代码逻辑也不复杂，开源项目并没有套路，初衷只是想给一些建站或者二次开发的朋友提供一个很好的轮子、而且后期的开源项目会包括VUE的后台模板、GOAPI、JavaCMS等项目，所以请尊重开源精神，我还能有点动力！
...
系统在全局中间件内置了扫描过滤的代码，如果你购买过宝塔或者其他防火墙的情况下，可以关闭这个状态

如果你没有使用的话，请保持打开这个，防止注入，当前这个模块期望你可以提交更好的PR，让代码本身的安全性更高

```injectablephp
文件 app/common/middleware/Security.php 内置了一些安全的扫描规则，
当然目前根据生产环境下各种AJAX的数据提交会产生一定的弊端。你可以根据这个规则扩展或者提交PR
...
8、权限类的优化，会员登录注册的流程，【目前的登录注册写在了User控制器，为了减少代码冗余，你可以将注册登录的代码，优化到Auth类】

    或者将User控制器直接抽离出来，当做一个模块去给全局调用；这样方便不同的模块，共享数据；
...
2、你可以在后台插件管理中安装你所需要的完整应用或者部分插件，注意安装完整应用的话是会直接覆盖index.php的，但是会生成一个文件`冲突TXT`以供查阅！
