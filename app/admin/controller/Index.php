...
class Index extends AdminController
{
    ...
    {
        if (request()->isPost()) {
            if (\app\common\library\Ftp::instance()->ftp_test(input())) {
    ...
    public function test() {
        ...
        $id = input('id/d');
        $result = Db::name('admin_rules')->where('pid',$id)->find();
        ...
        // 查询菜单
        $result = Db::name('admin_rules')->find($id);
        $url = str_replace('/index','/',$result['router']);
        $alias = substr($url,1,strlen($url)-1);
        ...
        $array = [
            ...
            [
                'pid'=>$result['id'],
                'title'=> '回收站',
                'router'=> $url.'recyclebin',
                'alias'=> $alias.'recyclebin',
                'type'=>2,
            ],
            [
                'pid'=>$result['id'],
        ...
        $db = new \app\common\model\system\AdminRules();
        foreach ($array as $value) {
            $value['icons'] = '';
            $value['sort'] = $db->count('id')+1;
