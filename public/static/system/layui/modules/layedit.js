...
layui.define(['layer', 'form','util'], function(exports){
  ...
  ,Edit = function(){
    ...
    that.config = {
      //默认工具bar
      tool: [
        'strong', 'italic', 'underline', 'del','code'
        ,'|'
        ,'left', 'center', 'right'
        ,'|'
    ...
    body.on('keydown', function(e){
      ...
      if(keycode === 13){
        ...
        if(parentNode.tagName.toLowerCase() === 'pre'){
          ...
          layer.msg('请暂时用shift+enter','warning');
          return false;
        }
    ...
    var body = this, index = layer.open({
      ...
      ,content: ['<ul class="layui-form" style="margin: 15px;">'
        ...
        ,'<button type="button" class="layui-btn layui-btn-primary"> 取消 </button>'
          ,'<button style="margin-left: 20px;" type="button" lay-submit lay-filter="layedit-code-yes" class="layui-btn layui-btn-normal"> 确定 </button>'
        ,'</li>'
      ,'</ul>'].join('')
      ,success: function(layero, index){
  ...
  ,tools = {
    ...
    ,help: '<i class="layui-icon layui-icon-tips layedit-tool-help" title="帮助" layedit-event="help"></i>'
  }
