...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}{{# if(d.data.className){ }} {{ d.data.className }}{{# } }}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'<div class="layui-table-box">'
      ...
      ,'<div class="layui-table-init" style="background-color: #fff;">'
        ,'<i class="layui-icon layui-icon-loading-exit layui-anim layui-anim-rotate layui-anim-loop"></i>'
      ,'</div>'
      ,'{{# } }}'
  ...
  Class.prototype.renderToolbar = function(){
    ...
    if(typeof options.defaultToolbar === 'object'){
      layui.each(options.defaultToolbar, function(i, item){
        ...
        if(thisItem){
          iconElem.push('<div class="layui-inline" lay-tips="'+ thisItem.title +'" data-offset="1" lay-event="'+ thisItem.layEvent +'">'
  ...
  Class.prototype.syncCheckAll = function(){
    ...
    if(table.checkStatus(that.key).isAll){
      ...
      // 全选样式
      checkAllElem.next().children().removeClass('layui-icon-subtraction').addClass('layui-icon-ok');
      syncColsCheck(true);
    } else {

        // 添加首次点击
        if(checkAllElem[0].checked || table.checkStatus(that.key).data.length == 1) {
          // 判断是否还有勾选的数据
          checkAllElem.prop('checked', table.checkStatus(that.key).data.length);
          that.renderForm('checkbox');
          checkAllElem.next().children().removeClass('layui-icon-ok').addClass('layui-icon-subtraction');
    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      switch(events){
        ...
        case 'LAYTABLE_PRINT': //打印
          ...
          printWin.close();
        break;
        case 'LAYTABLE_SEARCH': // 展开搜索
          var search = $('#laytable-search');
          if (search.is(':hidden')) {
            search.fadeIn(500);
          }else {
            search.fadeOut(500);
