...
layui.define('form', function(exports){
  ...
  ,ICON_ADD = 'layui-icon-right', ICON_SUB = 'layui-icon-down', ELEM_ENTRY = 'layui-tree-entry', ELEM_MAIN = 'layui-tree-main', ELEM_TEXT = 'layui-tree-txt', ELEM_PACK = 'layui-tree-pack', ELEM_SPREAD = 'layui-tree-spread'
  ,ELEM_LINE_SHORT = 'layui-tree-setLineShort', ELEM_SHOW = 'layui-tree-showLine', ELEM_EXTEND = 'layui-tree-lineExtend'
  ,ELEM_SUBTRACTION = 'layui-tree-subtraction'
  ,ELEM_TEXTCLICK = 'layui-tree-txt-click'
  //构造器
  ...
  Class.prototype.tree = function(elem, children){
    ...
    layui.each(data, function(index, item){
      if (typeof item.spread == true || options.spread == true) {
        item.spread = true
      }
          ...
          ,'<div class="layui-tree-main">'
            ...
            ,function(){
              if(options.showLine){
                ...
                if(hasChild){
                  return '<span class="layui-tree-iconClick layui-tree-line"><i class="layui-icon '+ (item.spread ? "layui-icon-down" : "layui-icon-right") +'"></i></span>';
                } else if (item.pid == 0)  {
                  return '<span class="layui-tree-iconClick"><i class="layui-icon layui-icon-file"></i></span>';
                  // return '<span class="layui-tree-iconClick"><i class="layui-icon" style="width:16px;display:inline-block;"></i></span>';
                }else {
                  return '<span class="layui-tree-iconClick"><i class="layui-icon "></i></span>';
                };
  ...
  Class.prototype.spread = function(elem, item){
    ...
    touchOpen.on('click', function(e){
      ...
      ,iconClick = touchOpen.children('.layui-icon')[0] ? touchOpen.children('.layui-icon') : touchOpen.find('.layui-tree-line').children('.layui-icon');
      ...
      if(!packCont[0]){
        ...
        if(elem.hasClass(ELEM_SPREAD)){
          ...
          if(options.accordion){
            ...
            sibls.find('.layui-tree-line').children('.layui-icon').removeClass(ICON_SUB).addClass(ICON_ADD);
          };
        };
      };
    });
    ...
    elemText.on('click', function(){
      ...
      // 增加hover选中效果
      $('.'+ELEM_TEXTCLICK).removeClass(ELEM_TEXTCLICK);
      othis.addClass(ELEM_TEXTCLICK);
  ...
  Class.prototype.setCheckbox = function(elem, item, elemCheckbox){
    ...
    if(typeof item.children === 'object' || elem.find('.'+ELEM_PACK)[0]){
      ...
      childs.each(function(){
        ...
        // 默认在初始化的时候，不选择子节点
        if(this.disabled || options.checkids.length > 0) 
          return; //不可点击则跳过
    ...
    var setParentsChecked = function(thisNodeElem){
      ...
      // 查找到checkbox的layui-tree-pack 也就是当前的所有子节点的那一块
      ,parentPack = thisNodeElem.parent('.'+ ELEM_PACK)

      // 往上层查询一下，获取到第一层的父类
      ,parentNodeElem = parentPack.parent()

      // 自身的那一块元素，往上一个，查找他的子元素，就是说获取到当前被点击的元素的
      // 那一块元素的父兄弟元素的方框。
      ...
      // 获取到所有的子节点总数
      var total = 0, allPacks = parentPack.find('input[same="layuiTreeCheck"]');
      allPacks.each(function(i,n) {
        if (this.checked) {
          total += 1;
        }
      })
      ...
      // 判断当前是否全部选中
      if (total != 0 && total < allPacks.length) {
        parentCheckbox.parents('.'+ELEM_MAIN).addClass(ELEM_SUBTRACTION);
      } else if (total == allPacks.length) {
        parentCheckbox.parents('.'+ELEM_MAIN).removeClass(ELEM_SUBTRACTION);
    ...
    entry.children('.layui-tree-btnGroup').on('click', '.layui-icon', function(e){
      ...
      if(type == 'add'){
        ...
        if(!packCont[0]){
          //若开启连接线，更改图标样式
          if(options.showLine){
            elemMain.find('.'+ICON_CLICK).addClass('layui-tree-line');
      ...
      //删除
        ...
        layer.confirm('确认删除该节点 "<span style="color: #999;">'+ (item.title || '') +'</span>" 吗？', function(index){
          ...
          if(elem.siblings('.'+ELEM_SET).children('.'+ELEM_ENTRY)[0]){
            ...
            //若开启了连接线
            if(options.showLine){
              prevDiv.find('.'+ICON_CLICK).removeClass('layui-tree-line');
  ...
  Class.prototype.events = function(){
    ...
    if (options.checkids.length > 0) {
      that.setChecked(options.checkids);
      // 后置为空
      this.config.checkids = [];
    }
  ...
  //得到选中节点
  Class.prototype.getChecked = function(specie){
    var that = this
    ,options = that.config
    ,checkId = []
    ,checkData = [];
    ...
    return specie ? checkId : checkData;
  };
  ...
  //获得选中的节点数据
  tree.getChecked = function(id,specie){
    var that = thisModule.that[id];
    return that.getChecked(specie);
  };
