layui.define(['jquery', 'layer', 'laypage'], function(exports) { 
    ...
    var fm = {
        config:{
            ...
            icon_url:'/static/admin/module/fileManager/ico/', // 暂时支持这些东西
            icon_array: ['ai','apk','bt','cad','code','dir','doc','eps','exe','fla',
                        'fonts','ipa','keynote','links','cloud','mm','mmap','mp3','mp4',
                        'number','pages','pdf','ppt','ps','rar','txt','visio','web','xls','xmind','zip'],
            btn_upload:true,
            btn_create:true
        }
        ,cache: {} //数据缓存
        ...
        layui.each(data,function(i,v){
            ...
            switch (v.type) {
                case 'dir':
                    ...
                    _type = 'dir';
                    break;
                default: // 默认情况下，如果是图片则直接显示，否则显示成其他的类型，但是还需要判断如果每次都是一个类型的话，其实则需要找到这个类型的数据才行
                    if (v.type == 'png' || v.type == 'gif' || v.type == 'jpg' || v.type == 'image') {
                        _img = '<img src="' + v.path + '" width="'+options.thumb['width']+'" height="'+options.thumb['height']+'" onerror=\'this.src="'+options.thumb['nopic']+'"\'  />';
                    } else {

                        if (options.icon_array.indexOf(v.type) === -1) {
                            v.type = 'cloud';
                        }

                        _img = '<div  style="width:'+options.thumb['width']+'px;height:'+options.thumb['height']+'px;line-height:'+options.thumb['height']+'px">';
                        _img += '<img src="' + options.icon_url + v.type + '.png"  onerror=\'this.src="'+options.thumb['nopic']+'"\' /></div>';
                    }
                    break;
            }
